...
#        self.values = {}
    ...
    def __unpack__(self, data):
        ...
        self.__unpacked_data_elms__ = data.unpack(self.__format__)
...
class SectionStructure(Structure):
    ...
    def __init__(self,parent):
        Structure.__init__(self,self.__IMAGE_SECTION_HEADER_format__)
    ...
    def set_relocations(self, data):
        ...
        self.relocations = []
        while offset < len(data):
            relocation = COFFRelocationStructure(self)
...
class SymbolStructure(Structure):
    ...
    def __unpack__(self, data):        
        ...
        if str(self.Name[:4]) == "\x00\x00\x00\x00":
            self.NameOffset = int(self.Name[4:8].unpack("<I")[0])
            try:
...
class COFFRelocationStructure(Structure):
    ...
    def apply(self,placeholder):
        ...
        coff = section.__parent__
        self.symbol = coff.SymbolTable[self.SymbolTableIndex]
        for (x,y) in self.symbol.__dict__.iteritems():
            if x != "__parent__":
                print "%-20s:\t\t%s" % (x,y)
        if self.symbol.StorageClass == IMAGE_SYMBOL_CLASSES['IMAGE_SYM_CLASS_EXTERNAL']:
            if self.symbol.SectionNumber != IMAGE_SYM_CLASS_NULL:
                return (coff.sections[self.symbol.SectionNumber],self.symbol.Value)
            else: #real extern:
                return self.symbol
...
class COFF:
    ...
    def parse_symboltable(self):
        ...
        self.COFFStringTable = \
            self.__data__[
                string_table_offset:
                string_table_offset + self.__data__[string_table_offset:string_table_offset+4].unpack("I")[0]]
...
class LIB:
    ...
    def __init__(self, name=None, data=None, fast_load=None):
        ...
        self.__data__ = lightstring.LightString(data)
    ...
    def __parse__(self):
        ...
        for i in xrange(self.SecondLinkerMember.NumberOfMembers):
            ...
            print "Parsing %s at %08x..." % (member_name,offset)
    ...
    def parse_first_linker_member(self, offset):
        ...
        offset += member_header.sizeof()
        number_of_symbols = int(self.__data__[offset:offset+4].unpack(">I")[0])
        ...
        for i in xrange(number_of_symbols):
            symbols.append((int(self.__data__[offset+4*i:offset+4*i+4].unpack(">I")[0]),strings[i]))
    ...
    def parse_second_linker_member(self, offset):
        ...
        #print "'%s'" % member_header.Name, "'%s'" % member_header.Date, "'%s'" % member_header.UserID, "'%s'" % member_header.GroupID, "'%s'" % member_header.Mode,"'%s'" % member_header.Size,  "'%s'" % member_header.EndOfHeader 
        offset += member_header.sizeof()
        number_of_members = int(self.__data__[offset:offset+4].unpack("<I")[0])
        offset += 4
        
        member_offsets = []
        for i in xrange(number_of_members):
            member_offsets.append(int(self.__data__[offset+4*i:offset+4*i+4].unpack("<I")[0]))
        ...
        symbols = []
        for i in xrange(number_of_symbols):
            symbols.append((strings[i], int(self.__data__[offset+2*i:offset+2*i+2].unpack("<H")[0])))
